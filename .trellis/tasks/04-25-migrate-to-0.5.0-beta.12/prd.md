# Migration Task: Upgrade to v0.5.0-beta.12

**Created**: 2026-04-25
**From Version**: 0.4.0-beta.10
**To Version**: 0.5.0-beta.12
**Assignee**: dawnli
...
- Activated this migration task by repairing `.trellis/.current-task`, which pointed at a missing older task.
- Added `implement.jsonl` and `check.jsonl` so Codex/Trellis hooks classify the task as ready.
- Ran `trellis update --migrate`; the project was already at `0.5.0-beta.12`, but the update preserved missing `.trellis/scripts/*.py` files as "Deleted by you".
- Restored the Trellis runtime scripts from the installed `@mindfoldhq/trellis@0.5.0-beta.12` template because the current workflow still requires `task.py`, `get_context.py`, and related helpers.
- Re-ran `trellis update`; it now reports "Already up to date!" with scripts recognized as unchanged.
- Verified `features.codex_hooks = true` is enabled in user-level Codex config.

## Verification

- `python3 ./.trellis/scripts/get_context.py --mode packages`
- `python3 ./.trellis/scripts/task.py list`
- `python3 ./.trellis/scripts/task.py validate .trellis/tasks/04-25-migrate-to-0.5.0-beta.12`
- `trellis update`
- Python syntax parse for `.trellis/scripts/**/*.py` and `.codex/hooks/**/*.py`
- JSON parse for `.codex/hooks.json` and `.trellis/.template-hashes.json`
- TOML parse for `.codex/config.toml` and `.codex/agents/*.toml`
- Codex `SessionStart` hook returns `Status: READY` for this migration task.
- Codex `UserPromptSubmit` hook returns the `migrate-to-0.5.0-beta.12 (in_progress)` workflow breadcrumb.
- Retired command / bare sub-agent scan found no active project references outside migration documentation/templates.

## Spec Update Decision

- No `.trellis/spec/` update was made. The new lesson is about Trellis migration recovery, while the current spec tree documents CTS backend/frontend implementation rules.
- The migration-specific recovery note is kept in this PRD so it remains tied to this one-off migration task.
