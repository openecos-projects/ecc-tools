...
set route_output_dir [file join $workspace_root route_ecc output]
set input_def [file join $route_output_dir gcd_route.def.gz]
set input_verilog [file join $route_output_dir gcd_route.v]
set input_db [file join $route_output_dir gcd_route_db]
...
set sta_use_spef 0
if {[info exists ::env(SKY130_STA_USE_SPEF)] && $::env(SKY130_STA_USE_SPEF) eq "1"} {
  set sta_use_spef 1
}
...
if {$sta_use_spef} {
  set spef_items [list [list "typical" $spef_file]]
} else {
  set spef_items [list [list "no_spef" ""]]
}
...
step_print_path "use_spef" $sta_use_spef
...
if {[llength $spef_items] <= 0} {
  error "no SPEF files found for STA"
}
