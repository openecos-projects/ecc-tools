# SKY130 ECC 脚本说明和用户手册

本文档说明 `scripts/design/sky130` 目录下的 GCD 参考设计脚本、运行方式、输入输出目录和调试开关。该 flow 面向 SKY130 PDK，使用 `ecc_bin` 依次执行 floorplan、fanout 修复、place、CTS、legalization、route、STA、DRC 和 filler。

## 目录结构

```text
scripts/design/sky130/
├── config/                 # 默认配置模板
...
`<step>_ecc` 包括 `Floorplan_ecc`、`fixFanout_ecc`、`place_ecc`、`CTS_ecc`、`legalization_ecc`、`route_ecc`、`sta_ecc`、`drc_ecc` 和 `filler_ecc`。
...
1. 在仓库根目录构建或准备 `ecc_bin`：
   ...
   `run_ecc.sh` 默认使用 `scripts/design/sky130/ecc_bin`。如果该文件不可执行，会尝试使用仓库下的 `bin/ecc_bin`。也可以通过 `ECC_BINARY=/path/to/ecc_bin` 指定。

2. 准备 workspace：

   默认 workspace home 为 `scripts/design/sky130/gcd/home`。命令行参数优先级高于环境变量：
...
3. 准备 SKY130 PDK：
   ...
   - Tech LEF: `lef/sky130_fd_sc_hs.tlef`
   - LEF: `lef/sky130_fd_sc_hs_merged.lef`、IO pad LEF、SRAM LEF 等
   - Liberty: `lib/sky130_fd_sc_hs__tt_025C_1v80.lib`、IO/SRAM Liberty 等
   - SDC: `<workspace_root>/origin/gcd.sdc`
   - SPEF: `<pdk_root>/spef/gcd.spef`
...
```bash
cd /nfs/home/huangzengrong/ecos_cloud/ecc_tool_refactor/scripts/design/sky130
...
| 7 | sta | `steps/sta.tcl` | `route_ecc/output/gcd_route.v`、`origin/gcd.sdc`、可选 SPEF | `read_netlist`、`read_liberty`、`read_sdc`、`report_timing` | `sta_ecc/output/gcd_sta.*`、`output/sta/no_spef/` 或 `output/sta/typical/` |
| 8 | drc | `steps/drc.tcl` | `route_ecc/output/gcd_route.def.gz` 或 `gcd_route_db` | `init_drc`、`run_drc` | `drc_ecc/output/gcd_drc.*`、`gcd_drc_db`、`report/drc.rpt` |
| 9 | filler | `steps/filler.tcl` | `drc_ecc/output/gcd_drc.def.gz` 或 `gcd_drc_db` | `run_filler` | `filler_ecc/output/gcd_filler.*`、`gcd_filler_db` |
...
`steps/sta.tcl` 默认不读取 SPEF，会把 STA 报告写到：

```text
sta_ecc/output/sta/no_spef/
```

如需读取 `steps/pdk.tcl` 设置的 `<pdk_root>/spef/gcd.spef`，设置：

```bash
SKY130_STA_USE_SPEF=1 ./run_ecc.sh sta gcd
```

开启后报告写到：

```text
sta_ecc/output/sta/typical/
```
...
- `Floorplan_ecc`、`fixFanout_ecc`、`place_ecc`、`CTS_ecc`、`legalization_ecc`、`route_ecc`、`sta_ecc`、`drc_ecc`、`filler_ecc`、`config`、`origin`、`home` 展开到 workspace 根目录。
- `lef`、`lib`、`spef`、`sdc`、`resource` 展开到 PDK 根目录。
...
```text
Floorplan fixFanout place CTS legalization route sta drc filler
```
...
默认 `sta` 不读取 SPEF。只有设置 `SKY130_STA_USE_SPEF=1` 时才会检查 `<pdk_root>/spef/gcd.spef`，如果该文件缺失，需要补齐 PDK SPEF 或关闭该环境变量。

### PDK 文件找不到

检查 workspace 中的 `pdk.root`，或确认默认 PDK 路径存在。`steps/pdk.tcl` 依赖 Tech LEF、LEF、Liberty、SDC 等路径，任一路径缺失都会导致初始化失败。
