...
bool DataManager::isNetConnected(IdbNet* net)
{
  ...
  return CheckInfo::kConnected == check_net.checkNetConnection();
}
...
#pragma omp parallel for schedule(dynamic)
  ...
  for (size_t i = 0; i <= 3; i++) {
    disconnected_net_list.insert(disconnected_net_list.end(), pin_num_disconnected_net_map[i].begin(), pin_num_disconnected_net_map[i].end());
  }
  ...
  return std::make_tuple(disconnected_net_list.empty(), disconnected_net_list, one_pin_net_list,
                         static_cast<int>(_design->get_net_list()->get_net_list().size()));
}
