...
#pragma omp parallel for schedule(dynamic, 16) num_threads(nthreads)
  for (long long i = 0; i < static_cast<long long>(signal_nets.size()); ++i) {
    ...
    checkNet(ac_model, design, signal_nets[static_cast<size_t>(i)], local_violations[tid]);
  }
...
void AntennaChecker::initLayers(ACModel& ac_model, idb::IdbDesign* design)
{
...
void AntennaChecker::checkNet(ACModel& ac_model, idb::IdbDesign* design, idb::IdbNet* net,
                              std::vector<ACViolation>& out_violations)
{
  ...
  auto createGateNode = [&](double gate_area, double diff_area, bool provides_diff) -> int {
    if (Utility::equalDoubleByError(gate_area, 0.0, ZH_ERROR) &&
        Utility::equalDoubleByError(diff_area, 0.0, ZH_ERROR) && !provides_diff) {
      return -1;
    }
  ...
  auto isRoutingLike = [&](idb::IdbLayer* layer, int order) -> bool {
    ...
    if (layer->get_type() == idb::IdbLayerType::kLayerMasterslice && order >= 0 &&
        static_cast<size_t>(order) < conductive_order.size() && conductive_order[order]) {
      return true;
    }
    return false;
  };
  ...
  auto processPin = [&](idb::IdbPin* pin, bool instance_pin) {
    ...
    if (!pin_shapes.empty()) {
      std::sort(pin_shapes.begin(), pin_shapes.end(), [](const ACPinShape& a, const ACPinShape& b) {
  ...
  auto edgeCmp = [](const ACEdge& a, const ACEdge& b) {
    return std::tie(a.time, a.a, a.b) < std::tie(b.time, b.a, b.b);
  };

  auto edgeEq = [](const ACEdge& a, const ACEdge& b) {
  ...
  for (int T = 0; T <= max_time; ++T) {
    ...
    const ACAntennaRule* routing_rule = pickRule(ac_model, T, true);
    const ACAntennaRule* cut_rule = pickRule(ac_model, T, false);
    ...
    for (int root : active_roots) {
      ...
      if (!cut_rects.empty()) {
        ...
        unionArea(cut_rects, micron_dbu, physical_cut_area_um);
      ...
      if (!routing_rects.empty()) {
        ...
        unionAreaPerimeter(routing_rects, micron_dbu, physical_metal_area_um, physical_metal_perimeter_um);
      ...
      const bool has_routing = !Utility::equalDoubleByError(metal_area_um, 0.0, ZH_ERROR) ||
                               !Utility::equalDoubleByError(side_area_um, 0.0, ZH_ERROR);
      ...
      if (Utility::equalDoubleByError(root_data.gate_area, 0.0, ZH_ERROR) &&
          Utility::equalDoubleByError(root_data.diff_area, 0.0, ZH_ERROR) && !root_data.diff_connected) {
        comps_without_gate.fetch_add(1, std::memory_order_relaxed);
        comps_by_root[root].clear();
        continue;
      }

      const bool diff_active = root_data.diff_connected ||
                               !Utility::equalDoubleByError(root_data.diff_area, 0.0, ZH_ERROR);
      ...
      auto emit = [&](const std::string& layer_name, ACViolationType type, double ratio, double threshold) {
        if (ratio > threshold) {
          ACViolation v;
          v.net_name = net_name;
      ...
      if (has_cut && cut_rule != nullptr) {
        if (!Utility::equalDoubleByError(cut_area_um, 0.0, ZH_ERROR)) {
          ...
          if (!Utility::equalDoubleByError(eff_gate, 0.0, ZH_ERROR)) {
            ...
            const double cut_car = Utility::getRatio(root_data.cum_cut_num, eff_gate);

            ACThresholdPick p = pickThreshold(cut_rule->area_ratio, cut_rule->diff_area_ratio, cut_rule->diff_area_ratio_pwl,
                                               root_data.diff_area, root_data.diff_connected);

            if (p.available) {
              emit(cut_rule->layer_name, p.is_diff ? ACViolationType::kAntennaDiffCutPar : ACViolationType::kAntennaCutPar, par_cut_check,
                   p.threshold);
            }
            ...
            if (p.available) {
              emit(cut_rule->layer_name, p.is_diff ? ACViolationType::kAntennaDiffCutCar : ACViolationType::kAntennaCutCar, cut_car,
                   p.threshold);
            }
          } else {
            comps_without_gate.fetch_add(1, std::memory_order_relaxed);
      ...
      if (has_routing && routing_rule != nullptr) {
        if (!Utility::equalDoubleByError(metal_area_um, 0.0, ZH_ERROR) ||
            !Utility::equalDoubleByError(side_area_um, 0.0, ZH_ERROR)) {
          ...
          if (!Utility::equalDoubleByError(eff_gate, 0.0, ZH_ERROR)) {
            ...
            const double car = Utility::getRatio(root_data.cum_area_num, eff_gate);
            const double csr = Utility::getRatio(root_data.cum_side_num, eff_gate);

            ACThresholdPick p = pickThreshold(routing_rule->area_ratio, routing_rule->diff_area_ratio, routing_rule->diff_area_ratio_pwl,
                                               root_data.diff_area, root_data.diff_connected);

            if (p.available) {
              emit(routing_rule->layer_name, p.is_diff ? ACViolationType::kAntennaDiffPar : ACViolationType::kAntennaPar, par_area_check,
                   p.threshold);
            }
            ...
            if (p.available) {
              emit(routing_rule->layer_name, p.is_diff ? ACViolationType::kAntennaDiffCar : ACViolationType::kAntennaCar, car, p.threshold);
            }
            ...
            if (p.available) {
              emit(routing_rule->layer_name, p.is_diff ? ACViolationType::kAntennaDiffPsr : ACViolationType::kAntennaPsr, par_side_check,
                   p.threshold);
            }
            ...
            if (p.available) {
              emit(routing_rule->layer_name, p.is_diff ? ACViolationType::kAntennaDiffCsr : ACViolationType::kAntennaCsr, csr, p.threshold);
            }
          } else {
            comps_without_gate.fetch_add(1, std::memory_order_relaxed);
          }
...
void AntennaChecker::readPinAntennaInfo(ACModel& ac_model, idb::IdbPin* pin, const bool instance_pin, double& gate_area,
...
const ACAntennaRule* AntennaChecker::pickRule(const ACModel& ac_model, const int layer_order, const bool routing) const
{
...
void AntennaChecker::unionArea(const std::vector<idb::IdbRect>& rects, int micron_dbu, double& area_um)
{
  double unused_perimeter_um = 0.0;
  unionAreaPerimeter(rects, micron_dbu, area_um, unused_perimeter_um);
}

void AntennaChecker::unionAreaPerimeter(const std::vector<idb::IdbRect>& rects, int micron_dbu, double& area_um, double& perimeter_um)
{
  area_um = 0.0;
  perimeter_um = 0.0;
...
ACThresholdPick AntennaChecker::pickThreshold(const double plain_ratio, const double diff_ratio,
...
{
  auto type_to_string = [](ACViolationType t) -> const char* {
    switch (t) {
