...
#define ZHUTIL (izh::Utility::getInst())
 ...
 public:
  ...
  static T getConfigValue(std::map<std::string, std::any>& config_map, const std::string& config_name, const T& default_value)
  {
    ...
    if (exist(config_map, config_name)) {
      ...
      ZHLOG.warn(Loc::current(), "The config '", config_name, "' uses the default value!");
      value = default_value;
    }
    return value;
  }
  ...
  static int32_t getBatchSize(int32_t total_size)
  {
    ...
    if (total_size < 0) {
      ZHLOG.error(Loc::current(), "The total of size < 0!");
  ...
  static bool isDivisible(double dividend, double divisor)
  {
    ...
    return equalDoubleByError(merchant, static_cast<int32_t>(merchant), ZH_ERROR);
  }
  ...
  static int32_t getOffset(const int32_t start, const int32_t end)
  {
    ...
    if (start < end) {
      ...
      ZHLOG.warn(Loc::current(), "The step == 0!");
    }
    return offset;
  }
  ...
  {
    if (flag_list.empty()) {
      ZHLOG.error(Loc::current(), "The flag list is empty!");
    }
    ...
    ZHLOG.error(Loc::current(), "The configuration file key '", key, "' does not exist!");
    return value;
  }
  ...
  {
    if (flag.empty()) {
      ZHLOG.error(Loc::current(), "The flag is empty!");
    }
    value = value[flag];
    if (!value.is_null()) {
      return value;
    }
    ZHLOG.error(Loc::current(), "The configuration file key '", flag, "' does not exist!");
    return value;
  }
  ...
  static double sigmoid(double value, double threshold)
  {
    ...
    if (std::isnan(result)) {
      ZHLOG.error(Loc::current(), "The value is nan!");
    }
    return result;
  }
  ...
  static T* getFileStream(std::string file_path)
  {
    ...
    if (!file->is_open()) {
      ZHLOG.error(Loc::current(), "Failed to open file '", file_path, "'!");
    }
    return file;
  }
  ...
  static bool isInteger(double a) { return equalDoubleByError(a, static_cast<int32_t>(a), ZH_ERROR); }

  static void checkFile(std::string file_path)
  {
    if (!std::filesystem::exists(file_path)) {
      ZHLOG.error(Loc::current(), "The file ", file_path, " does not exist!");
    }
  }
  ...
  static void createDir(std::string dir_path)
  {
    if (!std::filesystem::exists(dir_path)) {
      ...
      if (!std::filesystem::create_directories(dir_path, system_error)) {
        ZHLOG.error(Loc::current(), "Failed to create directory '", dir_path, "', system_error:", system_error.message());
      }
    }
  }
  ...
  static void changePermissions(const std::string& dir_path, std::filesystem::perms permissions)
  {
    ...
    if (system_error) {
      ZHLOG.error(Loc::current(), "Failed to change permissions for '", dir_path, "', system_error: ", system_error.message());
    }
  }
  ...
  static void removeDir(const std::string& dir_path)
  {
    ...
    if (std::filesystem::exists(dir_path, system_error)) {
      ...
      if (!std::filesystem::remove_all(dir_path, system_error)) {
        ZHLOG.error(Loc::current(), "Failed to remove directory '", dir_path, "'. Error: ", system_error.message());
      }
    }
  }
