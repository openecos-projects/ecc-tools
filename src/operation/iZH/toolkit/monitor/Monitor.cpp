...
std::string Monitor::getStatsInfo()
{
  ...
  stats_info = ZHUTIL.getString(" (elapsed = ", getElapsedTime(), ", cpu = ", getCPUTime(), ", mem = ", getUsageMemory(), ") ");
  updateStats();
  return stats_info;
}
...
{
  return ZHUTIL.formatSec(getCurrElapsedTime() - _init_elapsed_time);
}

std::string Monitor::getCPUTime()
{
  return ZHUTIL.formatSec(getCurrCPUTime() - _init_cpu_time);
}

std::string Monitor::getUsageMemory()
{
  return ZHUTIL.getString(ZHUTIL.formatByTwoDecimalPlaces(getCurrUsageMemory() - _init_usage_memory), "MB");
}
...
double Monitor::getCurrCPUTime()
{
  ...
  if (0 != getrusage(RUSAGE_SELF, &usage)) {
    ZHLOG.error(Loc::current(), "Unable to get rusage!");
  }
