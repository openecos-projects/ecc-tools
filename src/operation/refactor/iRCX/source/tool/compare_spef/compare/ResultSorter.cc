...
void sortRows(Result& result)
{
  ...
  std::sort(result.p2p_rows.begin(), result.p2p_rows.end(), [&](const ResistanceRow& lhs,
    ...
    const F64 lhs_report_test = std::round(lhs.test * 1000.0) / 1000.0;
    const F64 rhs_report_test = std::round(rhs.test * 1000.0) / 1000.0;
    const F64 lhs_report_reference = std::round(lhs.reference * 1000.0) / 1000.0;
    const F64 rhs_report_reference = std::round(rhs.reference * 1000.0) / 1000.0;
    if (lhs_report_test == rhs_report_test && lhs_report_reference == rhs_report_reference) {
      if (lhs.net != rhs.net) {
        return lhs.net < rhs.net;
      }
    }
    const F64 lhs_raw_rel = relative_value(lhs);
    const F64 rhs_raw_rel = relative_value(rhs);
