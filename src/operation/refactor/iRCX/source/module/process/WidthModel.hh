...
class WidthModel {
 public:
  ...
  void applyWidthVariation(Size corner_idx,
                             Size net_idx,
    ...
    for (Size edge_idx = 0; edge_idx < edge_count; ++edge_idx) {
      ...
      for (EdgeEtchInterval& etch_interval : edge_etch_intervals) {
        // TODO: 还不完善，只考虑了 vws 这种情况，甚至没考虑是否 CAPACITIVE_ONLY.
        for (const auto& etch_table : conductor_layer->get_etch_vws_list()) {
          // Low-side etch interval: lo_spacing is edge-to-edge in microns.
          Micron low_side_etch = 0;
          auto low_side_etch_opt =
              etch_table.query_etch(etch_interval.width, etch_interval.lo_spacing);
          ...
          auto high_side_etch_opt =
              etch_table.query_etch(etch_interval.width, etch_interval.hi_spacing);
