...
std::string Monitor::getStatsInfo()
{
  ...
  stats_info = FPUTIL.getString(" (elapsed = ", getElapsedTime(), ", cpu = ", getCPUTime(), ", mem = ", getUsageMemory(), ") ");
  updateStats();
  return stats_info;
}
...
{
  return FPUTIL.formatSec(getCurrElapsedTime() - _init_elapsed_time);
}

std::string Monitor::getCPUTime()
{
  return FPUTIL.formatSec(getCurrCPUTime() - _init_cpu_time);
}

std::string Monitor::getUsageMemory()
{
  return FPUTIL.getString(FPUTIL.formatByTwoDecimalPlaces(getCurrUsageMemory() - _init_usage_memory), "MB");
}
...
double Monitor::getCurrCPUTime()
{
  ...
  if (0 != getrusage(RUSAGE_SELF, &usage)) {
    FPLOG.error(Loc::current(), "Unable to get rusage!");
  }
