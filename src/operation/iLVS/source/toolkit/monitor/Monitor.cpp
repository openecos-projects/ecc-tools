...
std::string Monitor::getStatsInfo()
{
  ...
  stats_info = LVSUTIL.getString(" (elapsed = ", getElapsedTime(), ", cpu = ", getCPUTime(), ", mem = ", getUsageMemory(), ") ");
  updateStats();
  return stats_info;
}
...
{
  return LVSUTIL.formatSec(getCurrElapsedTime() - _init_elapsed_time);
}

std::string Monitor::getCPUTime()
{
  return LVSUTIL.formatSec(getCurrCPUTime() - _init_cpu_time);
}

std::string Monitor::getUsageMemory()
{
  return LVSUTIL.getString(LVSUTIL.formatByTwoDecimalPlaces(getCurrUsageMemory() - _init_usage_memory), "MB");
}
...
double Monitor::getCurrCPUTime()
{
  ...
  if (0 != getrusage(RUSAGE_SELF, &usage)) {
    LVSLOG.error(Loc::current(), "Unable to get rusage!");
  }
