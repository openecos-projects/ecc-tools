...
#define LVSLOG (ilvs::Logger::getInst())
...
class Logger
{
 ...
 private:
  ...
  void printLog(LogLevel log_level, Loc location, const T& value, const Args&... args)
  {
    ...
    std::string prefix = getString("[LVS ", getTimestamp(), " ", getCompressedBase62(std::stoul(getString(std::this_thread::get_id()))), " ", file_name, " ");
