...
void buildLayerComponentData(RVLayerData& merged_layer_data, const RVLayerData& source_layer_data)
{
  ...
  // Build per-polygon bounding boxes for connected-component discovery.
  std::vector<GTLRectInt> polygon_bounding_boxes(source_layer_data.polygon_pool.size());
  std::vector<int32_t> valid_polygon_ids;
  valid_polygon_ids.reserve(source_layer_data.polygon_pool.size());
  for (int32_t polygon_id = 0; polygon_id < static_cast<int32_t>(source_layer_data.polygon_pool.size()); ++polygon_id) {
    if (getPolygonBoundingBox(source_layer_data, source_layer_data.getPolygon(polygon_id), polygon_bounding_boxes[polygon_id])) {
      valid_polygon_ids.push_back(polygon_id);
    }
  }

  // Candidate discovery: plane sweep by default (set ECC_EOL_RTREE=1 to restore
  // per-polygon R-tree window queries). Both produce the same candidate pairs --
  ...
  if (std::getenv("ECC_EOL_RTREE") == nullptr) {
    ...
    // pair does -- and rect-rect closed overlap is four integer compares, with
    // cost independent of polygon complexity. Elements are swept by x with a
    // y-sorted flat active vector (binary-search insert, periodic compaction of
    // expired entries), keeping all access sequential.
    struct RectEntry {
      int32_t x_min;
      int32_t y_min;
      int32_t x_max;
    ...
    rect_entries.reserve(source_layer_data.max_rect_pool.size());
    for (int32_t polygon_id : valid_polygon_ids) {
      for (const MaxRectData& max_rect : source_layer_data.getMaxRects(source_layer_data.getPolygon(polygon_id))) {
        rect_entries.push_back(RectEntry{gtl::xl(max_rect.rect), gtl::yl(max_rect.rect), gtl::xh(max_rect.rect),
                                         gtl::yh(max_rect.rect), polygon_id});
      }
    }
    ...
    for (const RectEntry& entry : rect_entries) {
      // Closed y-overlap: other.y_min <= entry.y_max && other.y_max >= entry.y_min.
      for (const RectEntry& other : active_rects) {
        if (other.y_min > entry.y_max) {
          break;
        }
      ...
      const auto insert_pos = std::upper_bound(active_rects.begin(), active_rects.end(), entry.y_min,
                                               [](int32_t value, const RectEntry& e) { return value < e.y_min; });
      active_rects.insert(insert_pos, entry);
      if (++inserts_since_compact >= 256) {
        inserts_since_compact = 0;
        const auto expired_end = std::remove_if(active_rects.begin(), active_rects.end(),
                                                [&entry](const RectEntry& e) { return e.x_max < entry.x_min; });
  ...
  // Canonicalize component order by each component's minimum polygon ID (the
  // list front, since IDs are pushed in ascending order). union-find roots
  // depend on union order, so sorting by root would make downstream net IDs --
  // and through them rtree iteration order -- sensitive to candidate order.
