...
void RuleValidator::verifyMaxViaStack(RVCluster& rv_cluster)
{
  ...
  {
    auto cut_layer_it = routing_to_adjacent_cut_map.find(max_via_stack_rule.bottom_routing_layer_idx);
    if (cut_layer_it == routing_to_adjacent_cut_map.end() || cut_layer_it->second.empty()) {
      return;
    }
    const std::vector<int32_t>& cut_layer_idx_list = cut_layer_it->second;
    bottom_cut_layer_idx = *std::max_element(cut_layer_idx_list.begin(), cut_layer_idx_list.end());
  }
  int32_t top_cut_layer_idx = -1;
  {
    auto cut_layer_it = routing_to_adjacent_cut_map.find(max_via_stack_rule.top_routing_layer_idx);
    if (cut_layer_it == routing_to_adjacent_cut_map.end() || cut_layer_it->second.empty()) {
      return;
    }
    const std::vector<int32_t>& cut_layer_idx_list = cut_layer_it->second;
  ...
  for (auto& [cut_layer_idx, net_rect_map] : cut_net_rect_map) {
    ...
    for (std::pair<int32_t, PlanarRect>& net_stack_rect_pair : net_stack_rect_list) {
      ...
      for (auto& [curr_cut_layer_idx, net_stack_rect_map] : layer_net_stack_rect_map) {
        ...
        {
          auto routing_layer_it = cut_to_adjacent_routing_map.find(curr_cut_layer_idx);
