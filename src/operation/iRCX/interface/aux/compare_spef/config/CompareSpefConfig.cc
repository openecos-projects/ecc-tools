...
void NetConfigReader::addLine(Config& config, std::string_view raw_line) const
{
  ...
  const std::string line = string::trim(raw_line);
  if (line.empty() || string::startsWith(line, "//") || string::startsWith(line, "**")) {
    return;
  }

  if (string::startsWith(line, net_prefix)) {
    const std::string value = string::trim(line.substr(net_prefix.size()));
    if (!value.empty()) {
      config.net_names.emplace_back(value);
    }
    return;
  }
  if (string::startsWith(line, from_prefix)) {
    const std::string value = string::trim(line.substr(from_prefix.size()));
    if (!value.empty()) {
      config.from_pins.emplace_back(value);
    }
    return;
  }
  if (string::startsWith(line, to_prefix)) {
    const std::string value = string::trim(line.substr(to_prefix.size()));
    if (!value.empty()) {
      config.to_pins.emplace_back(value);
    }
    return;
  }
  if (string::startsWith(line, from_to_prefix)) {
    std::string_view payload = line;
    payload.remove_prefix(from_to_prefix.size());
    const std::string from_pin{string::takeToken(payload)};
    const std::string to_pin{string::takeToken(payload)};
