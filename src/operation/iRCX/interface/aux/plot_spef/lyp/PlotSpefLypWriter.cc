...
constexpr const char* kMacaronColors[] = {
    "#FADADD", "#F8BBD0", "#F6D6E8", "#EADCF8", "#DCCEF8", "#D7E3FC", "#CDE7F0", "#BDE0FE", "#A2D2FF", "#A8DADC", "#B2F7EF", "#B8F2E6", "#CDEAC0",
    "#D0F4DE", "#E9F5DB", "#F1F7B5", "#FFF3B0", "#FFF9C4", "#FFE5B4", "#FFD6A5", "#FBC4AB", "#FFADAD", "#FFCBD1", "#F7C6C7", "#DDBEA9", "#D7CCC8",
    "#E2F0CB", "#B5EAD7", "#C7CEEA", "#C3B1E1", "#F1C0E8", "#CFBAF0", "#A3C4F3", "#90DBF4", "#8EECF5", "#98F5E1", "#B9FBC0", "#FBF8CC", "#FDE4CF",
    "#FFCFD2", "#E4C1F9", "#D0BFFF", "#C1D3FE", "#ABC4FF", "#CAE9FF", "#ADE8F4", "#CAF0F8", "#B7E4C7", "#D8F3DC", "#EDEEC9", "#FFF1A8", "#FFE8A3",
    "#FFD6BA", "#FFB4A2", "#EAC7C7", "#E7D8C9", "#D5ECC2", "#C9E4DE", "#C9DAEA", "#DBCDF0", "#F2D7EE", "#F0D7A7", "#D8F3F0", "#DCEBFF",
};
...
auto collectLayers(const Model& model, const VisibilityReader& visibility) -> std::vector<int>
{
  std::set<int> layers;
...
auto hasVisibleEdgeOnLayer(const Model& model, const VisibilityReader& visibility, int layer) -> bool
{
  for (Size net_index = 0; net_index < model.nets.size(); ++net_index) {
    const auto& net = model.nets[net_index];
    if (!visibility.netVisible(net_index)) {
    ...
    for (Size resistor_index = 0; resistor_index < net.resistors.size(); ++resistor_index) {
      const auto& resistor = net.resistors[resistor_index];
      if (visibility.resistorVisible(net_index, resistor_index) && resistorPlotLayer(model, resistor) == layer) {
        return true;
      }
    }
  }
...
auto makePropertiesForLayer(const Model& model, const VisibilityReader& visibility, const Config& config, int layer, Size color_index)
    -> std::vector<LayerProperty>
{
  ...
  if (config.plotResistance()) {
    properties.push_back({.layer = layer, .data_type = kTextRes, .name = prefix + "TextRes", .frame_color = "#b000e8", .fill_color = "#b000e8"});
  }
  if (config.plotGroundCap()) {
    properties.push_back({.layer = layer, .data_type = kTextCg, .name = prefix + "TextCg", .frame_color = "#008a00", .fill_color = "#008a00"});
  }
  if (config.plotCouplingCap()) {
    properties.push_back({.layer = layer,
                          ...
                          .width = 1,
                          .filled = false});
    properties.push_back({.layer = layer, .data_type = kTextCc, .name = prefix + "TextCc", .frame_color = "#d06b00", .fill_color = "#d06b00"});
  }
  return properties;
}

auto makeProperties(const Model& model, const VisibilityReader& visibility, const Config& config) -> std::vector<LayerProperty>
{
  std::vector<LayerProperty> properties;
...
auto LypWriter::write(const Model& model, const Visibility& visibility, const Config& config) const -> bool
{
  const auto lyp_file = path::fileUnderDir(config.output_dir, string::identifier(model.design_name, "plot_spef"), ".lyp");
