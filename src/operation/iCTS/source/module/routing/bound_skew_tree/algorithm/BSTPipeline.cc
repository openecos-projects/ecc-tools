...
auto BstPipeline::bottomUpAllPairBased() -> void
{
  ...
  while (_impl._unmerged_nodes.size() > 1) {
    ...
    auto [erase_begin, erase_end] = std::ranges::remove_if(_impl._unmerged_nodes, [&](Area* node) -> bool { return node == left || node == right; });
...
auto BstPipeline::updateEmbeddedNodeTiming(Area* current) const -> void
{
  ...
  const auto delay_to_left
      = _impl.topDownEmbedding().pointDelayIncrease(parent_point, left_point, current->get_edge_len(kLeft), left->get_cap_load(), _impl._rc_pattern);
  const auto delay_to_right
      = _impl.topDownEmbedding().pointDelayIncrease(parent_point, right_point, current->get_edge_len(kRight), right->get_cap_load(), _impl._rc_pattern);
  ...
  if (TopDownEmbedding::pointSkew(parent_point) > _impl._skew_bound + kEpsilon) {
    ...
    CTSLOG.warn(Loc::current(), "skew is larger than skew bound with error: ", TopDownEmbedding::pointSkew(parent_point) - _impl._skew_bound);
