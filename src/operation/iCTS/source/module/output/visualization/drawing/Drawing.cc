...
auto DrawingBuilder::build(const DrawingInput& input) -> Drawing
{
  if (input.design == nullptr) {
    CTSLOG.error(Loc::current(), "DrawingBuilder: design is null.");
  }
  if (input.wrapper == nullptr) {
    CTSLOG.error(Loc::current(), "DrawingBuilder: wrapper is null.");
  }
  if (input.clock_layout == nullptr) {
    CTSLOG.error(Loc::current(), "DrawingBuilder: clock layout is null.");
  }
