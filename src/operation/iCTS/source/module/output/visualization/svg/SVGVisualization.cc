...
auto MakeFailure(std::string label, const std::filesystem::path& path, std::string view_label, std::string reason) -> VisualizationReportStatus
{
  CTSLOG.warn(Loc::current(), "CTS visualization report ", label, " failed: ", reason);
  return VisualizationReportStatus{.label = std::move(label), .path = path, .view_label = std::move(view_label), .success = false, .reason = std::move(reason)};
}

auto MakeSuccess(std::string label, const std::filesystem::path& path, std::string view_label, std::string reason) -> VisualizationReportStatus
{
  return VisualizationReportStatus{.label = std::move(label), .path = path, .view_label = std::move(view_label), .success = true, .reason = std::move(reason)};
}

auto CollectDrawablePoints(const std::vector<DrawingSegment>& segments, const std::vector<DrawingPin>& pins, const std::vector<DrawingInst>& insts,
                           const std::vector<DrawingLogicCell>& logic_cells) -> std::vector<Point<int>>
{
...
auto WriteSvgSegments(std::ofstream& output_stream, const visualization::detail::SvgTransform& transform, const std::vector<DrawingSegment>& segments,
                      bool flyline_view) -> void
{
  ...
  for (const auto& segment : segments) {
    ...
    output_stream << R"(<line x1=")" << begin_x << R"(" y1=")" << begin_y << R"(" x2=")" << end_x << R"(" y2=")" << end_y << R"(" stroke=")" << stroke_color
                  << R"(" stroke-width=")" << FormatSvgNumber(stroke_width) << R"(" stroke-opacity=")" << FormatSvgNumber(stroke_opacity) << '"';
    if (!dash_array.empty()) {
...
auto WriteSvgPins(std::ofstream& output_stream, const visualization::detail::SvgTransform& transform, const std::vector<DrawingPin>& pins) -> void
{
  ...
  for (const auto& pin : pins) {
    ...
    if (pin.driver) {
      output_stream << R"(<circle cx=")" << FormatSvgNumber(visualization::detail::MapX(transform, location.get_x())) << R"(" cy=")"
                    ...
                    << FormatSvgNumber(visualization::detail::kReportDriverRadius) << R"(" fill=")" << visualization::detail::kSvgColorDriverRoot
                    << R"(" stroke=")" << visualization::detail::kSvgColorNodeStroke << R"(" />
)";
    } else {
      output_stream << R"(<circle cx=")" << FormatSvgNumber(visualization::detail::MapX(transform, location.get_x())) << R"(" cy=")"
                    << FormatSvgNumber(visualization::detail::MapY(transform, location.get_y())) << R"(" r=")"
                    << FormatSvgNumber(visualization::detail::kSinkLoadRadius) << R"(" fill=")" << visualization::detail::kSvgColorSinkLoad
                    << R"(" fill-opacity="0.78" stroke=")" << visualization::detail::kSvgColorLoadStroke << R"(" />
)";
    }
  }
  output_stream << R"(</g>
...
auto WriteSvgInsts(std::ofstream& output_stream, const visualization::detail::SvgTransform& transform, const std::vector<DrawingInst>& insts) -> void
{
  ...
  for (const auto& inst : insts) {
    ...
    output_stream << R"(<rect x=")" << FormatSvgNumber(center_x - marker_size / 2.0) << R"(" y=")" << FormatSvgNumber(center_y - marker_size / 2.0)
                  << R"(" width=")" << FormatSvgNumber(marker_size) << R"(" height=")" << FormatSvgNumber(marker_size) << R"(" />
)";
  }
  output_stream << R"(</g>
)";
...
auto WriteSvgLegend(std::ofstream& output_stream, const visualization::detail::SvgTransform& transform) -> void
{
  ...
  output_stream << R"(<rect x=")" << FormatSvgNumber(legend_x - 8.0) << R"(" y=")" << FormatSvgNumber(legend_y - 16.0) << R"(" width="260" height=")"
                << FormatSvgNumber(legend_height) << R"(" rx="6" fill=")" << visualization::detail::kSvgColorLegendFill << R"(" fill-opacity=")"
                << FormatSvgNumber(visualization::detail::kSvgLegendFrameOpacity) << R"(" stroke=")" << visualization::detail::kSvgColorLegendStroke
                << R"(" />)";
  ...
  output_stream << R"(<text x=")" << FormatSvgNumber(legend_x + 20.0) << R"(" y=")" << FormatSvgNumber(row_y(1.0)) << R"(">routed segment</text>)";
  ...
  output_stream << R"(<text x=")" << FormatSvgNumber(legend_x + 20.0) << R"(" y=")" << FormatSvgNumber(row_y(2.0)) << R"(">flyline segment</text>)";
  ...
  output_stream << R"(<text x=")" << FormatSvgNumber(legend_x + 20.0) << R"(" y=")" << FormatSvgNumber(row_y(3.0)) << R"(">degraded segment</text>)";

  output_stream << R"(<circle cx=")" << FormatSvgNumber(legend_x + 7.0) << R"(" cy=")" << FormatSvgNumber(row_y(4.0) - 4.0) << R"(" r=")"
                << FormatSvgNumber(visualization::detail::kReportDriverRadius) << R"(" fill=")" << visualization::detail::kSvgColorDriverRoot << R"(" />)";
  output_stream << R"(<text x=")" << FormatSvgNumber(legend_x + 20.0) << R"(" y=")" << FormatSvgNumber(row_y(4.0)) << R"(">driver/root</text>)";

  output_stream << R"(<circle cx=")" << FormatSvgNumber(legend_x + 7.0) << R"(" cy=")" << FormatSvgNumber(row_y(5.0) - 4.0) << R"(" r=")"
                << FormatSvgNumber(visualization::detail::kSinkLoadRadius) << R"(" fill=")" << visualization::detail::kSvgColorSinkLoad << R"(" />)";
  output_stream << R"(<text x=")" << FormatSvgNumber(legend_x + 20.0) << R"(" y=")" << FormatSvgNumber(row_y(5.0)) << R"(">sink load</text>)";
  ...
  output_stream << R"(<rect x=")" << FormatSvgNumber(legend_x + 4.0) << R"(" y=")" << FormatSvgNumber(row_y(6.0) - 8.0) << R"(" width=")"
                ...
                << FormatSvgNumber(visualization::detail::kReportCtsBufferSize) << R"(" fill=")" << visualization::detail::kSvgColorBufferFillDefault
                << R"(" stroke=")" << visualization::detail::kSvgColorBufferStrokeDefault << R"(" stroke-width="1.0" />)";
  output_stream << R"(<text x=")" << FormatSvgNumber(legend_x + 20.0) << R"(" y=")" << FormatSvgNumber(row_y(6.0)) << R"(">CTS buffer</text>)";
  output_stream << R"(</g>
)";
}
...
auto logFailures(const std::vector<VisualizationReportStatus>& statuses) -> void
{
  for (const auto& status : statuses) {
    if (!status.success) {
      CTSLOG.warn(Loc::current(), "CTS SVG visualization '", status.label, "' failed at ", status.path.string(), ": ", status.reason);
    }
  }
}
...
auto EmitSvgVisualizations(const std::filesystem::path& visualization_dir, const Config& config, const Drawing& drawing) -> SvgVisualizationSummary
{
  const auto output_dir = ResolveVisualizationDir(config, visualization_dir) / "svg";

  std::vector<VisualizationReportStatus> statuses;
  if (!drawing.has_clocks) {
  ...
  if (drawing.design_segments.empty() && drawing.flyline_segments.empty()) {
    statuses = BuildUnavailableStatuses(output_dir, "CTS design contains no clock nets to visualize");
    logFailures(statuses);
    return SvgVisualizationSummary{.success = false};
  }

  statuses.push_back(WriteSvgFile(output_dir / kDesignSvgLabel, kDesignSvgLabel, "svg/design", drawing, drawing.design_segments, false));
  statuses.push_back(WriteSvgFile(output_dir / kFlylineSvgLabel, kFlylineSvgLabel, "svg/flyline", drawing, drawing.flyline_segments, true));
