...
#include "output/visualization/gds/layer/LayerPolicy.hh"
#include "output/visualization/gds/writer/GDSStream.hh"
...
auto appendSegments(const Config& config, GdsLibrary& library, LayerPolicy& layer_policy, const Drawing& model, ClockLayoutMode view) -> void
{
...
auto buildLibrary(const Config& config, const std::string& library_name, const std::string& structure_name, const Drawing& model, ClockLayoutMode view,
...
auto EmitGdsVisualizations(const std::filesystem::path& visualization_dir, const Config& config, const Drawing& drawing) -> GdsVisualizationSummary
{
  const auto output_dir = resolveVisualizationDir(config, visualization_dir) / "gds";
  ...
  if (!drawing.has_clocks || (drawing.design_segments.empty() && drawing.flyline_segments.empty())) {
    CTSLOG.warn(Loc::current(), "CTS GDS visualization skipped because no clock-tree view data is available.");
    return GdsVisualizationSummary{.success = false};
  }

  LayerPolicy design_layer_policy;
  auto design_library = buildLibrary(config, "CTS_DESIGN", "CTS_DESIGN", drawing, ClockLayoutMode::kDesign, design_layer_policy);
  LayerPolicy flyline_layer_policy;
  auto flyline_library = buildLibrary(config, "CTS_FLYLINE", "CTS_FLYLINE", drawing, ClockLayoutMode::kFlyline, flyline_layer_policy);
  ...
  const bool success = design_success && flyline_success && design_lyp_success && flyline_lyp_success;
  if (!success) {
    CTSLOG.warn(Loc::current(), "CTS GDS visualization generation failed in ", output_dir.string(), ".");
  }
  return GdsVisualizationSummary{.success = success};
}
