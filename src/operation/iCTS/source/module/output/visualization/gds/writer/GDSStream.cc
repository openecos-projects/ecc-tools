...
auto ensureParentDirectory(const std::filesystem::path& path) -> bool
{
  ...
  if (error) {
    CTSLOG.warn(Loc::current(), "GdsStream: failed to create directory ", parent_path.string(), ": ", error.message());
    return false;
  }
  return true;
}
...
auto makeGdsReal8(double value) -> std::array<uint8_t, 8>
{
  ...
  if (exponent <= 0 || exponent >= 128) {
    CTSLOG.warn(Loc::current(), "GdsStream: real8 value is outside GDSII exponent range.");
    return bytes;
  }
...
auto writeRecord(std::ofstream& stream, GdsRecord record, GdsDataType data_type, const std::vector<uint8_t>& payload) -> bool
{
  ...
  if (length > static_cast<std::size_t>(std::numeric_limits<uint16_t>::max())) {
    CTSLOG.warn(Loc::current(), "GdsStream: record payload is too large for one GDSII record.");
    return false;
  }
...
auto writeText(std::ofstream& stream, const GdsText& text) -> bool
{
  ...
  return writeNoDataRecord(stream, GdsRecord::kText) && writeLayerDatatype(stream, text.key, true)
         && writeInt32Record(stream, GdsRecord::kXy, {text.origin.get_x(), text.origin.get_y()}) && writeStringRecord(stream, GdsRecord::kString, text.text)
         && writeNoDataRecord(stream, GdsRecord::kEndEl);
}
...
auto GdsStream::writeBinary(const std::filesystem::path& path, const GdsLibrary& library) -> bool
{
  ...
  if (!stream.is_open()) {
    CTSLOG.warn(Loc::current(), "GdsStream: failed to open ", path.string(), " for binary GDSII output.");
    return false;
  }
  ...
  bool success = writeInt16Record(stream, GdsRecord::kHeader, {600}) && writeInt16Record(stream, GdsRecord::kBgnLib, makeDatePayload())
                 ...
                 && writeInt16Record(stream, GdsRecord::kBgnStr, makeDatePayload()) && writeStringRecord(stream, GdsRecord::kStrName, library.structure_name);
...
auto GdsStream::writeLayerProperties(const std::filesystem::path& path, const std::vector<GdsLayerProperty>& layers) -> bool
{
  ...
  if (!stream.is_open()) {
    CTSLOG.warn(Loc::current(), "GdsStream: failed to open ", path.string(), " for layer properties.");
    return false;
  }
