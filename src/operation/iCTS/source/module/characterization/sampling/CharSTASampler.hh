...
class CharStaSampler
{
 public:
  ...
  auto characterizeTopology(unsigned length_idx, const TopologyDesc& topo, const std::vector<std::string>& buf_masters, BuildProgress& build_progress) -> void;

 private:
  auto sampleFeasibleTopology(unsigned length_idx, const ::icts::PatternId& pid, const TopologyDesc& topo, const std::vector<std::string>& buf_masters,
                              const PatternFeasibility& feasibility, BuildProgress& build_progress) -> void;
  auto sampleLoadSlews(unsigned length_idx, const ::icts::PatternId& pid, double effective_load_pf, double load_pf, double driven_cap_pf,
