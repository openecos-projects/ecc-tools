...
auto CharStaSampler::sampleFeasibleTopology(unsigned length_idx, const ::icts::PatternId& pid, const TopologyDesc& topo,
                                            const std::vector<std::string>& buf_masters, const PatternFeasibility& feasibility, BuildProgress& build_progress)
    -> void
{
  if (!_impl.circuitBuilder().createCharCircuit(topo, buf_masters)) {
    build_progress.skipped_sta_samples += _impl._loads_to_test.size() * _impl._slews_to_test.size();
    return;
  }
  ...
  for (const double load_pf : _impl._loads_to_test) {
    ...
    build_progress.max_observed_driven_cap_idx = std::max(build_progress.max_observed_driven_cap_idx, cap_lattice.coveringIndex(driven_cap_pf));
    ...
    sampleLoadSlews(length_idx, pid, effective_load, load_pf, driven_cap_pf, power_context_ready, build_progress);
  }

  _impl.circuitBuilder().destroyCharCircuit();
}

auto CharStaSampler::sampleLoadSlews(unsigned length_idx, const ::icts::PatternId& pid, double effective_load_pf, double load_pf, double driven_cap_pf,
                                     bool& power_context_ready, BuildProgress& build_progress) -> void
{
  _impl.circuitBuilder().setCharParasitics(effective_load_pf);
  ...
  for (const double input_slew_ns : _impl._slews_to_test) {
    ...
    const auto stored_sample_indices = tryMakeStoredSampleIndices(input_slew_idx, load_cap_idx, sample_result.output_slew_ns, driven_cap_pf, build_progress);
    if (!stored_sample_indices.has_value()) {
      continue;
    }

    const ::icts::CharCore core(stored_sample_indices->input_slew_idx, stored_sample_indices->output_slew_idx, stored_sample_indices->driven_cap_idx,
                                stored_sample_indices->load_cap_idx, sample_result.delay_ns, power_w, pid, source_boundary_net_switch_power_w);
