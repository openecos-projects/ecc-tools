...
auto EmitSourceTrunkSummary(const SourceTrunkInput& input, const SourceTrunkSummary& summary) -> void
{
  EmitLogTable(Loc::current(), "Source Trunk Summary", {"Property", "Value"},
               {{"Clock", input.log_context.clock_name},
                {"Net", input.log_context.clock_net_name},
                {"Sink Domain", input.log_context.sink_domain},
                {"Stage", ToString(summary.stage)},
                {"Success", ToLogTableCell(summary.success)},
                {"Selected Depth", summary.selected_depth.has_value() ? ToLogTableCell(*summary.selected_depth) : "n/a"},
                {"Selected Levels", ToLogTableCell(summary.selected_level_count)},
                {"Inserted Buffers", ToLogTableCell(summary.inserted_buffer_count)},
                {"Inserted Nets", ToLogTableCell(summary.inserted_net_count)},
                {"Boundary Relaxed", ToLogTableCell(summary.used_boundary_relaxation)},
...
auto BuildSourceTrunkTree(const SourceTrunkInput& input) -> SourceTrunkBuild
{
  if (input.config == nullptr) {
    CTSLOG.error(Loc::current(), "Topology source trunk build requires an explicit config.");
  }
  if (input.design == nullptr) {
    CTSLOG.error(Loc::current(), "Topology source trunk build requires an explicit design.");
  }
  if (input.wrapper == nullptr) {
    CTSLOG.error(Loc::current(), "Topology source trunk build requires an explicit wrapper.");
  }
  if (input.fast_sta == nullptr) {
    CTSLOG.error(Loc::current(), "Topology source trunk build requires an explicit FastSTA.");
  }
  if (input.source_net == nullptr) {
    CTSLOG.error(Loc::current(), "Topology source trunk build requires an explicit source net.");
  }
  ...
  if (clock_source == nullptr) {
    ...
    CTSLOG.warn(Loc::current(), "Topology: top-level source-to-root synthesis failed because clock source is null.");
    return result;
  }
  ...
  if (valid_root_inputs.empty()) {
    ...
    CTSLOG.warn(Loc::current(), "Topology: top-level source-to-root synthesis failed because no root inputs are available.");
    return result;
  }
  ...
  if (valid_root_inputs.size() == 1U) {
    ...
    EmitLogTable(Loc::current(), "Source Trunk Segment Selection", {"Metric", "Value"},
                 {{"Length (um)", ToLogTableCell(segment_build.summary.length_um)},
                  {"Length Index", ToLogTableCell(segment_build.summary.length_idx)},
                  {"Strict Candidates", ToLogTableCell(segment_build.summary.strict_candidate_count)},
                  {"Relaxed Candidates", ToLogTableCell(segment_build.summary.relaxed_candidate_count)},
                  {"Required Load Cap Index", ToLogTableCell(segment_build.summary.required_load_cap_idx)},
                  {"Source Drive Cap Index", ToLogTableCell(segment_build.summary.source_drive_cap_idx)},
                  {"Boundary Relaxed", ToLogTableCell(segment_build.summary.used_boundary_relaxation)}});
    RecordTopSegmentBuild(result, segment_build);
    EmitSourceTrunkSummary(input, result.summary);
    return result;
  }
