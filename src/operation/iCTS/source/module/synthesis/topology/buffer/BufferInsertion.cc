...
auto ConnectNet(const TopologyNetConnectionInput& input) -> void
{
  if (input.net == nullptr) {
    CTSLOG.error(Loc::current(), "Topology buffer insertion: net connection target is null.");
  }
  auto& net = *input.net;
...
{
  if (input.net == nullptr) {
    CTSLOG.error(Loc::current(), "Topology buffer insertion: net reconnection target is null.");
  }
  auto& net = *input.net;
