...
auto ClockDistribution::prepare(const ClockDistributionInput& input) -> std::optional<ClockDistributionContext>
{
  if (input.design == nullptr) {
    CTSLOG.error(Loc::current(), "ClockDistribution: design is null.");
  }
  if (input.clock == nullptr) {
    CTSLOG.error(Loc::current(), "ClockDistribution: clock is null.");
  }
  if (input.wrapper == nullptr) {
    CTSLOG.error(Loc::current(), "ClockDistribution: Wrapper is null.");
  }
  if (input.status_recorder == nullptr) {
    CTSLOG.error(Loc::current(), "ClockDistribution: status recorder is null.");
  }
  ...
  auto& status_recorder = *input.status_recorder;
