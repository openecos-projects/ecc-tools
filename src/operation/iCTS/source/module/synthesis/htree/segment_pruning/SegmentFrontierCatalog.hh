...
class SegmentCandidateFrontierSet
{
 public:
  ...
  auto require(SegmentFrontierKind kind) const -> const std::vector<SegmentChar>&
  {
    ...
    if (frontier == nullptr) {
      CTSLOG.error(Loc::current(), "HTree: required segment frontier kind was not synthesized.");
    }
    return *frontier;
  }
...
class SegmentFrontierCatalog
{
 public:
  ...
  auto require(unsigned length_idx, SegmentFrontierKind kind) const -> const std::vector<SegmentChar>&
  {
    ...
    if (frontier == nullptr) {
      CTSLOG.error(Loc::current(), "HTree: required segment frontier length/kind was not synthesized.");
    }
    return *frontier;
  }
