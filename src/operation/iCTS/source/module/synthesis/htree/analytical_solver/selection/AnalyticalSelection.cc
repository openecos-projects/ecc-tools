...
auto RequireMaterializedAnalyticalChar(const AnalyticalValidatedCandidate& candidate) -> const HTreeTopologyChar&
{
  if (!candidate.candidate.materialized_char.has_value()) {
    CTSLOG.error(Loc::current(), "HTree: missing materialized analytical topology char.");
  }
...
auto CollectAnalyticalDelayPowerParetoRefs(const std::vector<AnalyticalValidatedCandidate>& candidates) -> std::vector<const AnalyticalValidatedCandidate*>
{
  ...
  std::ranges::sort(sorted_candidates, [](const AnalyticalValidatedCandidate* lhs, const AnalyticalValidatedCandidate* rhs) -> bool {
    if (lhs == nullptr || rhs == nullptr) {
      CTSLOG.error(Loc::current(), "HTree: null analytical candidate during Pareto sorting.");
    }
  ...
  std::ranges::sort(pareto_front, [](const AnalyticalValidatedCandidate* lhs, const AnalyticalValidatedCandidate* rhs) -> bool {
    if (lhs == nullptr || rhs == nullptr) {
      CTSLOG.error(Loc::current(), "HTree: null analytical candidate during Pareto delay ordering.");
    }
    return ShouldPreferAnalyticalCandidate(lhs->candidate, rhs->candidate);
  });
  return pareto_front;
    ...
    -> const AnalyticalValidatedCandidate*
{
  ...
  for (const auto* candidate : pareto_front) {
    if (candidate == nullptr || !candidate->candidate.materialized_char.has_value()) {
      CTSLOG.error(Loc::current(), "HTree: invalid analytical Pareto candidate during power guard calculation.");
    }
  ...
  if (std::isfinite(guarded_power_limit_w) && guarded_power_limit_w > 0.0) {
    for (const auto* candidate : pareto_front) {
      if (candidate == nullptr || !candidate->candidate.materialized_char.has_value()) {
        CTSLOG.error(Loc::current(), "HTree: invalid analytical Pareto candidate during power-guarded delay selection.");
      }
...
auto MakeAnalyticalDepthSummary(const CandidateBuildEvaluation& evaluation, const SinkLoadRegionLegalitySummary& sink_legality) -> DepthSummary
{
  return htree::DepthSummary{
      ...
      .split_group_count = sink_legality.split_group_count,
      .split_extra_buffer_count = sink_legality.split_extra_buffer_count,
      .split_local_depth = sink_legality.split_local_depth,
      .split_triggered_by_fanout = sink_legality.split_triggered_by_fanout,
      .split_triggered_by_capacitance = sink_legality.split_triggered_by_capacitance,
