...
{
  if (input.routing_layer <= 0) {
    CTSLOG.error(Loc::current(), "HTree: routing layer is not configured for root-driver compensation.");
  }
...
auto DbuToUm(const RootDriverCompensationInput& input, int64_t dbu) -> double
{
  if (input.dbu_per_um <= 0) {
    CTSLOG.error(Loc::current(), "HTree: DBU-per-micron is unavailable for root-driver compensation.");
  }
                             ...
                             RootDriverCompensationStats& stats) -> RootClosureWireEstimate
{
  ...
  CTSLOG.warn(Loc::current(), "HTree: root-closure FLUTE estimate was unavailable; using HPWL estimate.");
                                   ...
                                   const BufferPatternLibrary& segment_pattern_library) -> RootClosureLoadSignature
{
  ...
  for (const auto segment_pattern_id : level_segment_pattern_ids) {
    ...
    if (segment_pattern == nullptr) {
      CTSLOG.error(Loc::current(), "HTree: candidate segment pattern metadata is missing during root-load signature build.");
    }
                                    ...
                                    RootDriverCompensationStats& stats) -> RootClosureLoadEstimate
{
  ...
  for (const auto segment_pattern_id : level_segment_pattern_ids) {
    const auto* segment_pattern = segment_pattern_library.find(segment_pattern_id);
    if (segment_pattern == nullptr) {
      CTSLOG.error(Loc::current(), "HTree: candidate segment pattern metadata is missing during root-load resolution.");
    }
