...
// `max_unit_um` is the electrical ceiling on the length unit (see
// ResolveMaxCharacterizationSegmentLengthUm). Omitted when the caller cannot supply it.
auto ResolveCharacterizationGridPlan(const Config& config, const std::vector<double>& requested_lengths_um,
                                     const std::optional<double>& max_unit_um = std::nullopt) -> CharacterizationGridPlan;
...
auto ResolveCharacterizationGridPlan(const CharBuilder::Config& config, const std::vector<double>& direct_lengths_um,
                                     const std::vector<double>& coverage_lengths_um, const std::optional<double>& max_unit_um = std::nullopt)
