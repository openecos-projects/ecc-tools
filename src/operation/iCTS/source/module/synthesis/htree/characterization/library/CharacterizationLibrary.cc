...
auto CharacterizationLibrary::ensure(const CharBuilder::Input& input, const CharBuilder::Config& config) -> EnsureSummary
{
  ...
  if (_char_builder.get_segment_chars().empty() || _char_builder.get_wirelength_unit_um() <= 0.0) {
    CTSLOG.warn(Loc::current(), "CharacterizationLibrary: characterization did not produce reusable segment chars.");
    _ready = false;
...
{
  if (runtime_input.config == nullptr) {
    CTSLOG.error(Loc::current(), "CharacterizationLibrary: runtime config is null.");
  }
  if (runtime_input.wrapper == nullptr) {
    CTSLOG.error(Loc::current(), "CharacterizationLibrary: runtime wrapper is null.");
  }
  if (runtime_input.fast_sta == nullptr) {
    CTSLOG.error(Loc::current(), "CharacterizationLibrary: runtime FastSTA is null.");
  }
...
auto CharacterizationLibrary::buildRuntimeConfig(const Config& config) -> CharBuilder::Config
{
  ...
  if (routing_layers.empty() || routing_layers.front() == 0U) {
    CTSLOG.error(Loc::current(), "CharacterizationLibrary: routing layer must be configured before characterization.");
  }
