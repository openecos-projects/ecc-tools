...
{
 public:
  CtsClockIdbWriter(Wrapper& wrapper, Design& design) : _wrapper(&wrapper), _design(&design) {}
 ...
 private:
  ...
  auto createOrUpdateClockTreeInst(Inst* inst) -> idb::IdbInstance*
  {
    ...
    if (_wrapper->_idb_layout == nullptr || _wrapper->_idb_layout->get_cell_master_list() == nullptr) {
      CTSLOG.warn(Loc::current(), "CTS iDB clock-tree materialization failed for inst \"", inst->get_name(),
    ...
    if (idb_inst == nullptr) {
      ...
      if (idb_inst == nullptr) {
        CTSLOG.warn(Loc::current(), "CTS iDB clock-tree materialization failed: failed to allocate iDB inst \"", inst->get_name(), "\".");
        return nullptr;
      }
  ...
  auto findOrCreateClockTreeIdbNet(Net* net, const std::string& default_net_name) -> idb::IdbNet*
  {
    ...
    if (net_name.empty()) {
      ...
      CTSLOG.warn(Loc::current(), "CTS iDB clock-tree materialization failed: clock tree net name is empty.");
      return nullptr;
    }
    ...
    if (idb_net == nullptr) {
      ...
      CTSLOG.warn(Loc::current(), "CTS iDB clock-tree materialization failed: failed to create iDB net \"", net_name, "\".");
      return nullptr;
    }
    _wrapper->crossRef(idb_net, net);
    return idb_net;
  ...
  auto rewriteClockTreeIdbNetPins(idb::IdbNet* idb_net, Net* cts_net, const ClockTreeIdbMaterializationScope& scope) -> bool
  {
    ...
    if (driver == nullptr) {
      ...
      CTSLOG.warn(Loc::current(), "CTS iDB clock-tree materialization failed for net \"", net_name, "\": CTS driver pin is null.");
      return false;
    }
    ...
    for (auto* load : cts_net->get_loads()) {
      if (load == nullptr) {
        ...
        CTSLOG.warn(Loc::current(), "CTS iDB clock-tree materialization failed for net \"", net_name, "\": CTS load pin is null.");
        return false;
      }
...
auto Wrapper::writeClock(Design& design, Clock& clock) -> bool
{
  return CtsClockIdbWriter(*this, design).writeClocksDetailed({&clock}).success;
}

auto Wrapper::writeClocksDetailed(Design& design, const std::vector<Clock*>& clocks) -> WrapperWriteSummary
{
  return CtsClockIdbWriter(*this, design).writeClocksDetailed(clocks);
}

auto Wrapper::writeClocks(Design& design, const std::vector<Clock*>& clocks) -> bool
{
  return writeClocksDetailed(design, clocks).success;
}
