...
{
 public:
  CtsClockReader(Wrapper& wrapper, Design& design) : _wrapper(&wrapper), _design(&design) {}
  ...
  auto readClocks(const std::vector<std::pair<std::string, std::string>>& clock_net_pairs) -> bool
  {
    ...
    if (idb_design == nullptr) {
      CTSLOG.warn(Loc::current(), "CTS clock read failed: iDB design is null.");
      return false;
    }

    auto* idb_net_list = idb_design->get_net_list();
    if (idb_net_list == nullptr) {
      CTSLOG.warn(Loc::current(), "CTS clock read failed: iDB net list is null.");
      return false;
    }
 ...
 private:
  ...
  auto buildInstFromIdbInst(idb::IdbInstance* idb_inst) -> Inst*
  {
    ...
    if (cts_inst == nullptr) {
      CTSLOG.warn(Loc::current(), "CTS clock read failed: failed to create CTS inst \"", inst_name, "\".");
      return nullptr;
    }
  ...
  auto buildOrFindPinFromIdbPin(idb::IdbPin* idb_pin, Inst* cts_inst) -> Pin*
  {
    ...
    if (!_design->indexPin(cts_pin)) {
      CTSLOG.warn(Loc::current(), "CTS clock read failed: failed to index CTS pin \"", Design::getPinFullName(cts_pin), "\".");
      return nullptr;
    }
    return cts_pin;
  }
  ...
  auto buildPinFromIdbPin(idb::IdbPin* idb_pin, Inst* cts_inst) -> Pin*
  {
    ...
    if (_design->findPin(pin_full_name) != nullptr) {
      CTSLOG.warn(Loc::current(), "CTS clock read failed: duplicate CTS pin \"", pin_full_name, "\".");
      return nullptr;
    }

    auto* cts_pin = _design->makePin(pin_name);
    if (cts_pin == nullptr) {
      CTSLOG.warn(Loc::current(), "CTS clock read failed: failed to create CTS pin \"", pin_full_name, "\".");
      return nullptr;
    }
  ...
  auto findIdbInstOrError(const std::string& clock_name, const std::string& inst_name) -> idb::IdbInstance*
  {
    ...
    if (idb_inst == nullptr) {
      CTSLOG.warn(Loc::current(), "CTS clock read failed for clock \"", clock_name, "\": preclustered sink anchor inst \"", inst_name,
                  "\" is not found in iDB.");
      return nullptr;
    }
    return idb_inst;
  ...
  static auto findIdbInstPinOrError(const std::string& clock_name, idb::IdbInstance* idb_inst, const std::string& pin_name) -> idb::IdbPin*
  {
    ...
    if (idb_pin == nullptr) {
      CTSLOG.warn(Loc::current(), "CTS clock read failed for clock \"", clock_name, "\": preclustered sink anchor pin \"", pin_name,
  ...
  auto buildNetFromIdbNet(idb::IdbNet* idb_net) -> Net*
  {
    ...
    if (cts_net == nullptr) {
      CTSLOG.warn(Loc::current(), "CTS clock read failed: failed to create CTS net \"", idb_net->get_net_name(), "\".");
      return nullptr;
    }
...
auto Wrapper::readClocks(Design& design, const std::vector<std::pair<std::string, std::string>>& clock_net_pairs) -> bool
{
  return CtsClockReader(*this, design).readClocks(clock_net_pairs);
}

auto Wrapper::readTraceClockTargets(Design& design, const std::vector<ClockTraceClockTarget>& clock_targets) -> bool
{
  return CtsClockReader(*this, design).readTraceClockTargets(clock_targets);
}
