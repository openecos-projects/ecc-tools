...
struct FastStaClockBuildResult
{
  std::optional<FastStaClockId> clock_id = std::nullopt;
  std::string failure_reason;

  auto ok() const -> bool { return clock_id.has_value(); }
};
...
class FastSTA
{
 public:
  ...
  auto buildClockContext(const FastStaClockBuildInput& input) -> FastStaClockBuildResult;
  ...
  auto injectNetRouteTree(FastStaClockId clock_id, const Net& net, const ClockSteinerTree<int>& route_tree, FastStaClockNetRcTreeCounts& rc_tree_counts)
      -> bool;
  ...
  auto queryPower(FastStaClockId clock_id) const -> std::optional<FastStaPowerSummary>;
