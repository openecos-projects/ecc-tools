...
auto FastStaDmpCeff::calcLoadDelaySlew(const FastStaDmpDriverResult& driver_timing, double elmore_delay_ns, const FastStaLibertyCell* load_cell)
...
auto FastStaDmpCeff::calcInputPortDelaySlew(double input_slew_ns, double elmore_delay_ns, FastStaTransition transition, const FastStaLibertyCell* load_cell)
    -> FastStaDmpLoadResult
{
  const auto elmore_ns = std::max(0.0, elmore_delay_ns);
