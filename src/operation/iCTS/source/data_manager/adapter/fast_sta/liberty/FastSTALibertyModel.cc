...
auto FastStaLibertyTable::valid() const -> bool
{
  if (values.empty() || axes.size() > 2U || !std::ranges::all_of(values, [](double value) -> bool { return std::isfinite(value); })) {
    return false;
  }

  std::size_t expected_value_count = 1U;
  for (const auto& axis : axes) {
    if (axis.kind == FastStaLibertyAxisKind::kUnknown || axis.values.empty()
        || !std::ranges::all_of(axis.values, [](double value) -> bool { return std::isfinite(value); })
        || std::ranges::adjacent_find(axis.values, [](double lhs, double rhs) -> bool { return lhs >= rhs; }) != axis.values.end()) {
      return false;
    }
    expected_value_count *= axis.values.size();
  }
  return values.size() == expected_value_count;
}
...
auto FastStaLibertyTable::lookup(double input_slew_ns, double output_load_pf) const -> std::optional<double>
{
  ...
  if (axes.size() == 1U) {
    ...
    return linearInterpolate(axis.values.at(low), axis.values.at(high), values.at(low), values.at(high), axisQueryValue(axis, input_slew_ns, output_load_pf));
  }
