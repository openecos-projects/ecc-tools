...
auto FastStaClockTree::applyRouteGeometry(FastStaClockContext& context, const FastStaClockRouteGeometry& route_geometry) -> void
{
  ...
  if (route_geometry_dbu_per_um <= 0) {
    CTSLOG.error(Loc::current(), "FastStaClockTree: clock route geometry DBU-per-micron is invalid.");
  }
  if (context.dbu_per_um <= 0) {
    CTSLOG.error(Loc::current(), "FastStaClockTree: Fast STA DBU-per-micron is invalid.");
  }
  if (context.dbu_per_um != route_geometry_dbu_per_um) {
    CTSLOG.error(Loc::current(), "FastStaClockTree: Fast STA DBU-per-micron (", context.dbu_per_um, ") does not match clock route geometry DBU-per-micron (",
                 route_geometry_dbu_per_um, ").");
  }
  if (context.routing_layer <= 0) {
    CTSLOG.error(Loc::current(), "FastStaClockTree: routing layer must be initialized before route-geometry RC extraction.");
  }
