...
auto SdcSubsetEvaluator::evaluateSet(const std::vector<ParsedWord>& words) -> SdcValue
{
  ...
  std::ranges::transform(values, std::back_inserter(joined_values), [](const SdcValue& value) -> std::string { return ValueToString(value); });
...
auto SdcSubsetEvaluator::evaluateAllClocks() -> SdcValue
{
  ...
  for (const auto& clock_period : _clock_period_by_name) {
    clock_names.emplace_back(clock_period.first);
  }
