...
auto Trim(const std::string& text) -> std::string
{
  ...
  const auto last = std::ranges::find_if_not(text.rbegin(), text.rend(), [](unsigned char ch) -> bool { return std::isspace(ch) != 0; }).base();
...
auto ValueToString(const SdcValue& value) -> std::string
{
  if (!value.objects.empty()) {
    ...
    std::ranges::transform(value.objects, std::back_inserter(patterns), [](const SdcObjectRef& object) -> std::string { return object.pattern; });
