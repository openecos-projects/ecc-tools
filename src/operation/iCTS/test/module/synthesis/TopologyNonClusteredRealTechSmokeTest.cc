...
TEST(TopologyRealTechSmokeTest, NonClusteredModeSkipsClusterBuffersAndUsesUnrestrictedHTreeFrontier)
{
  ...
  CTSDM.getConfig().set_enable_sink_clustering(false);
  CTSDM.getConfig().set_htree_topology_tolerance(0.1);
  ASSERT_FALSE(CTSDM.getConfig().is_enable_sink_clustering());
  ...
  CTSLOG.info(icts::Loc::current(), "Topology smoke scenario: non_clustered_mode, clock=", selected_clock_data.clock_name,
