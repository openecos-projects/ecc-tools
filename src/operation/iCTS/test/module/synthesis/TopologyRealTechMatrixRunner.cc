...
auto PopulateDistanceDeltaStats(const std::vector<double>& relaxed_distances, const std::vector<double>& exact_distances,
                                TopologyToleranceComparisonResult& comparison_result) -> void
{
  const std::size_t distance_count = std::min(relaxed_distances.size(), exact_distances.size());
  ...
  for (std::size_t distance_index = 0; distance_index < distance_count; ++distance_index) {
    const double delta = relaxed_distances.at(distance_index) - exact_distances.at(distance_index);
    delta_sum += delta;
...
{
  return CTSDM.getWrapper().queryConfiguredClockRouteSegmentRc(CTSDM.getConfig());
}
...
auto EvaluateBpBeTopFullSinkNonClusteredExperimentMatrix() -> TopologyMatrixRunResult
{
  ...
  for (const unsigned wirelength_iterations : kBpBeTopExperimentIterations) {
    for (const unsigned slew_cap_steps : kBpBeTopExperimentSteps) {
      ...
      CTSDM.getConfig().set_wirelength_iterations(wirelength_iterations);
      CTSDM.getConfig().set_slew_steps(slew_cap_steps);
      CTSDM.getConfig().set_cap_steps(slew_cap_steps);
      ...
      AppendCaseFailures(wirelength_iterations, slew_cap_steps, result, runtime_s, matrix_result.failure_messages);
    }
  }
...
auto EvaluateArm9FullSinkTopologyToleranceComparison() -> TopologyToleranceComparisonResult
{
  ...
  CTSDM.getConfig().set_wirelength_iterations(3U);
  CTSDM.getConfig().set_slew_steps(15U);
  CTSDM.getConfig().set_cap_steps(15U);

  for (const double topology_tolerance : {0.1, 0.0}) {
    CTSDM.getConfig().set_htree_topology_tolerance(topology_tolerance);
    icts::Topology::Config config;
    SetEnableSinkClustering(config, false);
