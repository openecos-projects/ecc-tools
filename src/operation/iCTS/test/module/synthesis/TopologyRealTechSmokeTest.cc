...
TEST(TopologyRealTechSmokeTest, ClusteredModeBuildsCentroidBuffersAndUsesUnrestrictedHtreeFrontier)
{
  ...
  CTSDM.getConfig().set_max_fanout(smoke::kSynthesisTestDefaultMaxFanout);
  CTSDM.getConfig().set_enable_sink_clustering(true);
  CTSDM.getConfig().set_htree_topology_tolerance(0.1);
  ASSERT_EQ(CTSDM.getConfig().get_max_fanout(), smoke::kSynthesisTestDefaultMaxFanout);
  ASSERT_TRUE(CTSDM.getConfig().is_enable_sink_clustering());
  ASSERT_DOUBLE_EQ(CTSDM.getConfig().get_htree_topology_tolerance(), 0.1);
  ...
  CTSLOG.info(icts::Loc::current(), "Topology smoke scenario: clustered_mode, clock=", selected_clock_data.clock_name,
              ", loads=", selected_clock_data.sinks.size(), ".");
...
TEST(TopologyRealTechSmokeTest, ClusteredModeForceBranchBufferedRealtechSmoke)
{
  ...
  ASSERT_FALSE(artifact_paths.output_dir.empty());
  CTSLOG.openLogFileStream(artifact_paths.cts_log.string());
  CTSLOG.info(icts::Loc::current(), "Topology smoke scenario: clustered_mode_force_branch_buffered, clock=", selected_clock_data.clock_name,
              ", loads=", selected_clock_data.sinks.size(), ".");
