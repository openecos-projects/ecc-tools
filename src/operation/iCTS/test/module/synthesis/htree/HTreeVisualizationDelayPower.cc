...
auto BuildDelayPowerPoints(const icts::htree::DiagnosticBuild& result) -> std::vector<DelayPowerPoint>
{
  ...
  points.push_back(DelayPowerPoint{
      ...
      .normalized_delay = NormalizeMetric(result.output.best_char->get_delay(), result.output.best_char->get_delay(), result.output.best_char->get_delay()),
      .normalized_power = NormalizeMetric(result.output.best_char->get_power(), result.output.best_char->get_power(), result.output.best_char->get_power()),
      .is_pareto = true,
      .is_selected = true,
  });
...
auto BuildDelayPowerChoiceSummary(const icts::htree::DiagnosticBuild& result) -> std::optional<DelayPowerChoiceSummary>
{
  ...
  const std::size_t selection_pool_size
      = observation.used_boundary_relaxation ? observation.selected_candidate_frontier_entry_count : observation.selected_feasible_frontier_entry_count;
...
auto WriteDelayPowerParetoSvg(const std::filesystem::path& path, const icts::htree::DiagnosticBuild& result) -> bool
{
  ...
  output_stream << icts::visualization::detail::kSvgOpenTagPrefix << FormatSvgNumber(width) << icts::visualization::detail::kSvgHeightTag
                << FormatSvgNumber(height) << icts::visualization::detail::kSvgViewBoxPrefix << FormatSvgNumber(width) << ' ' << FormatSvgNumber(height)
  ...
  output_stream << R"(<text x="36" y="46" font-size="12">Each point is one sink-load-region-legal frontier entry. Lower-left is better.</text>)";
  output_stream << R"(<rect x=")" << FormatSvgNumber(plot_left) << R"(" y=")" << FormatSvgNumber(plot_top) << R"(" width=")" << FormatSvgNumber(plot_width)
                << R"(" height=")" << FormatSvgNumber(plot_height) << R"(" fill="#ffffff" fill-opacity="0.85" stroke="#d0d7de" />)";
  ...
  for (int tick = 0; tick <= 5; ++tick) {
    ...
    output_stream << R"(<line x1=")" << FormatSvgNumber(grid_x) << R"(" y1=")" << FormatSvgNumber(plot_top) << R"(" x2=")" << FormatSvgNumber(grid_x)
                  << R"(" y2=")" << FormatSvgNumber(plot_bottom) << R"(" stroke="#edf2f7" stroke-width="1" />)";
    output_stream << R"(<line x1=")" << FormatSvgNumber(plot_left) << R"(" y1=")" << FormatSvgNumber(grid_y) << R"(" x2=")" << FormatSvgNumber(plot_right)
                  << R"(" y2=")" << FormatSvgNumber(grid_y) << R"(" stroke="#edf2f7" stroke-width="1" />)";

    output_stream << R"(<text x=")" << FormatSvgNumber(grid_x - 8.0) << R"(" y="456" font-size="11">)" << FormatSvgNumber(normalized_value) << R"(</text>)";
    output_stream << R"(<text x="52" y=")" << FormatSvgNumber(grid_y + 4.0) << R"(" font-size="11">)" << FormatSvgNumber(normalized_value) << R"(</text>)";
  }

  output_stream << R"(<line x1=")" << FormatSvgNumber(plot_left) << R"(" y1=")" << FormatSvgNumber(plot_bottom) << R"(" x2=")" << FormatSvgNumber(plot_right)
                << R"(" y2=")" << FormatSvgNumber(plot_bottom) << R"(" stroke="#4a5568" stroke-width="1.4" />)";
  output_stream << R"(<line x1=")" << FormatSvgNumber(plot_left) << R"(" y1=")" << FormatSvgNumber(plot_bottom) << R"(" x2=")" << FormatSvgNumber(plot_left)
                << R"(" y2=")" << FormatSvgNumber(plot_top) << R"(" stroke="#4a5568" stroke-width="1.4" />)";
  output_stream << R"(<text x=")" << FormatSvgNumber((plot_left + plot_right) * 0.5 - 58.0) << R"(" y="488" font-size="13">normalized delay</text>)";
  ...
  for (const auto& point : points) {
    ...
    output_stream << R"(<circle cx=")" << FormatSvgNumber(center_x) << R"(" cy=")" << FormatSvgNumber(center_y) << R"(" r=")" << FormatSvgNumber(radius)
                  << R"(" fill=")" << fill_color << R"(" stroke=")" << stroke_color << R"(" stroke-width="1.2">)";
...
auto BuildReport(const std::string& scenario_name, const std::string& input_summary, const HTreeArtifactPaths& paths, const std::vector<icts::Pin*>& loads,
                 const icts::htree::DiagnosticBuild& result) -> std::string
{
  ...
  report << "char_grid_unit_um=" << result.diagnostics.char_wirelength_unit_um << ", iterations=" << result.diagnostics.char_wirelength_iterations
         << ", distinct_level_bins=" << result.diagnostics.char_unique_level_bins << ", adapted=" << (result.diagnostics.char_grid_adapted ? "true" : "false")
         << "\n";
  ...
  report << "frontier_solution_count=" << delay_power_points.size()
         << ", pareto_solution_count=" << std::ranges::count_if(delay_power_points, [](const DelayPowerPoint& point) -> bool { return point.is_pareto; })
         << "\n";
  ...
  if (selection_summary.has_value()) {
    ...
    report << "delay_power_selection_summary selection_policy="
           ...
           << ", selection_pool_size=" << selection_summary->selection_pool_size << ", pareto_solution_count=" << selection_summary->pareto_solution_count
           << ", pareto_power_median_index=";
    ...
    if (result.output.best_char.has_value()) {
      report << ", selected_pattern_id=" << result.output.best_char->get_pattern_id().local_id << ", selected_delay=" << result.output.best_char->get_delay()
             << ", selected_power=" << result.output.best_char->get_power();
    }
    report << "\n";
    report << std::setprecision(3);
  }
