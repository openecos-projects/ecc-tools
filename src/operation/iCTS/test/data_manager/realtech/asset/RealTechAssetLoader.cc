...
#include "data_manager/realtech/asset/RealTechAssetLoader.hh"
...
namespace icts_test::data_manager::realtech::asset {
namespace {
...
auto ResolveExistingPath(const std::filesystem::path& base_path, const std::array<std::string_view, 2>& relative_candidates) -> std::filesystem::path
{
...
auto ResolveExistingPath(const std::filesystem::path& base_path, const std::array<std::string_view, N>& relative_candidates) -> std::filesystem::path
{
  for (const auto& relative_path : relative_candidates) {
    const auto candidate = base_path / relative_path;
    if (std::filesystem::exists(candidate)) {
...
auto LoadRealTechAssets(const RealTechAssets& assets, std::string& error) -> bool
{
  ...
  const auto work_dir = icts_test::toolkit::io::ResolveClusteringOutputDir() / "real_tech_workspace";
  ...
  dm_config.set_output_path((icts_test::toolkit::io::ResolveClusteringOutputDir() / "real_tech_output").string());
...
auto BuildRealTechSetupState() -> RealTechSetupState
{
  ...
  state.output_dir = icts_test::toolkit::io::ResolveClusteringOutputDir();
  std::error_code error_code;
