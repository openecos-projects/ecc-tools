...
{
 public:
  DRPatchState() = default;
  DRPatchState(const DRPatchState&) = default;
  DRPatchState(DRPatchState&&) = default;
  DRPatchState& operator=(const DRPatchState&) = default;
  DRPatchState& operator=(DRPatchState&&) = default;
  ~DRPatchState() = default;
  // single task
  ...
  void resetTask()
  {
    set_curr_patch_task(nullptr);
    get_routing_patch_list().clear();
    get_patch_violation_list().clear();
    get_tried_fix_violation_set().clear();
  }
  void release() { *this = DRPatchState(); }
