...
class DRModelResult
{
 public:
  ...
  const std::map<int32_t, std::vector<Segment<LayerCoord>>>& get_net_detailed_result_map() const { return _net_detailed_result_map; }
  const std::map<int32_t, std::vector<EXTLayerRect>>& get_net_detailed_patch_map() const { return _net_detailed_patch_map; }
