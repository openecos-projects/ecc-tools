...
std::string Monitor::getStatsInfo()
{
  ...
  stats_info = PWUTIL.getString(" (elapsed = ", getElapsedTime(), ", cpu = ", getCPUTime(), ", mem = ", getUsageMemory(), ") ");
  updateStats();
  return stats_info;
}
...
{
  return PWUTIL.formatSec(getCurrElapsedTime() - _init_elapsed_time);
}

std::string Monitor::getCPUTime()
{
  return PWUTIL.formatSec(getCurrCPUTime() - _init_cpu_time);
}

std::string Monitor::getUsageMemory()
{
  return PWUTIL.getString(PWUTIL.formatByTwoDecimalPlaces(getCurrUsageMemory() - _init_usage_memory), "MB");
}
...
double Monitor::getCurrCPUTime()
{
  ...
  if (0 != getrusage(RUSAGE_SELF, &usage)) {
    PWLOG.error(Loc::current(), "Unable to get rusage!");
  }
