...
std::string Monitor::getStatsInfo()
{
  ...
  stats_info = EMIRUTIL.getString(" (elapsed = ", getElapsedTime(), ", cpu = ", getCPUTime(), ", mem = ", getUsageMemory(), ") ");
  updateStats();
  return stats_info;
}
...
{
  return EMIRUTIL.formatSec(getCurrElapsedTime() - _init_elapsed_time);
}

std::string Monitor::getCPUTime()
{
  return EMIRUTIL.formatSec(getCurrCPUTime() - _init_cpu_time);
}

std::string Monitor::getUsageMemory()
{
  return EMIRUTIL.getString(EMIRUTIL.formatByTwoDecimalPlaces(getCurrUsageMemory() - _init_usage_memory), "MB");
}
...
double Monitor::getCurrCPUTime()
{
  ...
  if (0 != getrusage(RUSAGE_SELF, &usage)) {
    EMIRLOG.error(Loc::current(), "Unable to get rusage!");
  }
