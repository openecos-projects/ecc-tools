...
unsigned StaBuildGraph::buildConst(StaGraph* the_graph, Instance* inst) {
  ...
  FOREACH_INSTANCE_PIN(inst, pin) {
    ...
    if (!pin->get_net() || (port_func && (liberty_expr_func_is_one(port_func) ||
                                          liberty_expr_func_is_zero(port_func)))) {
      (*the_vertex)->set_is_const();
      the_graph->addConstVertex(*the_vertex);

      if (port_func) {
        if (liberty_expr_func_is_one(port_func)) {
          (*the_vertex)->set_is_const_vdd();
        } else if (liberty_expr_func_is_zero(port_func)) {
