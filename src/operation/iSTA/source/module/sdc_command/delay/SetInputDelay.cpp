...
unsigned TclSetInputDelay::exec()
{
  ...
  if (getOptionOrArg("-clock_fall")->is_set_val() && !clock_option->is_set_val()) {
    setTclError("set_input_delay -clock_fall requires -clock");
    return 0;
  }
  if (has_reference_pin
      && (getOptionOrArg("-network_latency_included")->is_set_val() || getOptionOrArg("-source_latency_included")->is_set_val())) {
    setTclError("set_input_delay -reference_pin cannot be used with -network_latency_included or -source_latency_included");
    return 0;
  }
  ...
  if (has_reference_pin) {
    ...
    if (reference_pins.size() != 1) {
      setTclError("set_input_delay -reference_pin requires exactly one pin or port");
      return 0;
    }
  ...
  for (const std::string& port_name : ports) {
    getOrCreatePortConstraint(database, port_name).set_input_delays(delays, getOptionOrArg("-add_delay")->is_set_val());
  }
  return 1;
}
