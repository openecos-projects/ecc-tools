...

{
  ...
  *tbl << "Status" << instProperty.get_status_str(inst.get_status()) << TABLE_ENDLINE;
  if (inst.is_placed() || inst.is_fixed()) {
    *tbl << "Position" << ReportBase::format("(%d,%d)", inst.get_coordinate()->get_x(), inst.get_coordinate()->get_y()) << TABLE_ENDLINE;
  }
  *tbl << "Orient" << getInstOrient(inst.get_orient()) << TABLE_ENDLINE;

  auto* box = inst.get_bounding_box();
  *tbl << "Box" << ReportBase::format("(%d, %d, %d, %d)", box->get_low_x(), box->get_low_y(), box->get_high_x(), box->get_high_y())
       << TABLE_ENDLINE;
  *tbl << "Box Size" << ReportBase::format("%d, %d", box->get_width(), box->get_height()) << TABLE_ENDLINE;
