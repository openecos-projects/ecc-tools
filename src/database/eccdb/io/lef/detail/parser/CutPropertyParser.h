...
bool parse_lef58_cutclass(Iterator beg, Iterator end, std::vector<lef58_cutclass>& vec)
{
  ...
  const static qi::rule<Iterator, cut::lef58_cutclass(), space_type> cutclass_rule
  ...
  const static qi::rule<Iterator, std::vector<cut::lef58_cutclass>(), space_type> rule = cutclass_rule % qi::eps;
...
bool parse_lef58_enclosureedge(Iterator beg, Iterator end, std::vector<lef58_enclosureedge>& vec)
{
  ...
  const static qi::rule<Iterator, cut::lef58_enclosureedge_width(), space_type> width_struct_rule
      = width_rule >> parallel_rule >> within_rule >> -qi::string("EXCEPTEXTRACUT") >> -double_ >> -qi::string("EXCEPTTWOEDGES")
        >> -double_;
  const static qi::rule<Iterator, cut::lef58_enclosureedge_convexcorners(), space_type> convexconers_rule
      = lit("CONVEXCORNERS") >> double_ >> double_ >> lit("PARALLEL") >> double_ >> lit("LENGTH") >> double_;

  const static qi::rule<Iterator, cut::lef58_enclosureedge, space_type> rule
...
{
  const static qi::rule<Iterator, std::string(), ascii::space_type> value_string = lexeme[+(char_ - char_(" ;\n"))];
  const static qi::rule<Iterator, cut::lef58_eolspacing_toclass, space_type> toclass_rule
      = lit("TO") >> value_string >> double_ >> double_;
  const static qi::rule<Iterator, cut::lef58_eolspacing, space_type> eolspacing_rule
...
bool parse_lef58_spacingtable(Iterator beg, Iterator end, lef58_spacingtable& spacingtable)
{
  ...
  const static qi::rule<Iterator, cut::lef58_spacingtable_layer(), space_type> layer_rule
  ...
  const static qi::rule<Iterator, cut::lef58_spacingtable_cutspacing(), space_type> cutspacing_rule
