...
{
  const char* configured_root = std::getenv("OPENROAD_SOURCE_DIR");
  const auto root = configured_root != nullptr && configured_root[0] != '\0'
                        ? std::filesystem::path{configured_root}
                        : std::filesystem::path{ECC_TOOLS_SOURCE_DIR}.parent_path() / "OpenROAD";
  const auto lef = root / "src/odb/test/data/gscl45nm.lef";
