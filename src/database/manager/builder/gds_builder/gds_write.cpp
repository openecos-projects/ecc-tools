...
bool Def2GdsWrite::resolveVirtualLayer(const string& layer_name, const string& purpose, int32_t& layer_id, int32_t& datatype)
{
  ...
  // DIEAREA and COMP are synthetic layers generated by ECC.  Innovus
  // technology maps do not normally contain these names, but they do contain
  // ZERO/drawing.  Reuse that canonical zero layer for the synthetic geometry.
  if ((layer_name == "DIEAREA" || layer_name == "COMP") && _layer_map.resolve("ZERO", "drawing", value)) {
    layer_id = static_cast<int32_t>(value.layer);
    datatype = static_cast<int32_t>(value.datatype);
    return true;
  }
...
int32_t Def2GdsWrite::write_specialnet_wire_segment_rect(gdstk::Cell* gds_cell, IdbSpecialWireSegment* segment)
{
  if (segment->get_layer() == nullptr || segment->get_delta_rect() == nullptr) {
    ECCLOG.info(ecc::Loc::current(), "No special wire segment rect...");
    return kDbFail;
  }
...
int32_t Def2GdsWrite::write_harden_macro_pins()
{
  ...
  if (design == nullptr || layout == nullptr) {
    ECCLOG.warn(ecc::Loc::current(), "Write harden macro pins failed...");
    return kDbFail;
  }
...
int32_t Def2GdsWrite::write_harden_macro_obs()
{
  ...
  if (design == nullptr || layout == nullptr || die == nullptr || layers == nullptr) {
    ECCLOG.warn(ecc::Loc::current(), "Write harden macro obs failed...");
    return kDbFail;
  }
      ...
      for (auto* pin : io_pins->get_pin_list()) {
        ...
        for (auto* layer_shape : pin->get_port_box_list()) {
          ...
          for (auto* port_rect : layer_shape->get_rect_list()) {
            ...
            ecc_solver::GtlRect bloat_rect(port_rect->get_low_x(), port_rect->get_low_y(), port_rect->get_high_x(),
